...
(function () {
	...
	var clean = function () {
		ctx.clearRect(0, 0, scr.width, document.body.scrollHeight);
		for (var i = 0, rb; rb = objects[i++];) {
			if (rb.pos[1] > document.body.scrollHeight) {
				i--;
				objects.splice(i, 1);
			}
		}
	...
	var resize = function () {
		...
		objects.push(new Rectangle(false, scr.width * 0.5, document.body.scrollHeight, scr.width, 4, 0, 0)); // floor
	}
	var drop = function () {
		for (var i = 0; i < 50; i++) {
			dropc++;
